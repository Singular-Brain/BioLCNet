...
from utils.monitors import RewardMonitor

from utils.learning import PostPre
...
from utils.visualization import plot_convergence_and_histogram, plot_locally_connected_output_weights, plot_locally_connected_feature_maps
...
class BioLCNet(Network):
    def __init__(
        ...
        **kwargs,
        ...
        if kwargs['variant'] == 'scalar':
            assert self.has_decision_period == True, 'Decision period is necessary for scalar variant'

        if self.online == False:
            assert self.has_decision_period == True, 'Decision period is necessary for offline learning'
        ...
        if self.has_decision_period == True:
            ...
            assert self.decision_period > 0, "Decision period should be greater than zero"
        ...
        main = AdaptiveLIFNodes(shape= [n_channels_lc, conv_size, conv_size], traces=True, tc_trace=tc_trace,
                             traces_additive = trace_additive,tc_theta_decay = tc_theta_decay, theta_plus = theta_plus)
        
        self.add_layer(main, name="main")

        ### connections 
        LC = LocalConnection2D(inp, main, filter_size, stride, in_channels, n_channels_lc, input_shape=(crop_size, crop_size), nu = _pair(nu_LC), update_rule = PostPre, wmin = wmin, wmax= wmax, norm = norm_factor_LC)
        ...
        if LC_weights_path:
            ...
            print('LC pre-trained weights not loaded. Training will be end-to-end and will take more time!')
        ...
        w_inh_LC = torch.zeros(n_channels_lc, main_width, main_width, n_channels_lc, main_width, main_width)
        ...
        ### main to output
        out = LIFNodes(n= n_neurons, traces=True,traces_additive = trace_additive, tc_trace=tc_trace, tc_theta_decay = tc_theta_decay, theta_plus = theta_plus)

        self.add_layer(out, "output")

        last_main_out = Connection(self.final_connection_source, out, nu = nu_Output, update_rule = MSTDP, wmin = wmin, wmax= wmax)
        ...
        **kwargs
        ...
        for (i, datum) in enumerate(dataloader):
            ...
            if self.norm_factor_LC is not None:
                ...
                convg_lc1=  1 - ( torch.mean((w_lc - self.wmin)*(self.wmax-w_lc)) / ((mean_norm_factor_lc - self.wmin)*(self.wmax - mean_norm_factor_lc)) )
            ...
            ### Tensorboard and plotting eligibility traces
            ...
            #Plot_et.plot()    
            print("\nRunning accuracy: " + "{:.2f}".format(acc) + "%, " + "Current val accuracy: " + "{:.2f}".format(val_acc) + "%, ")
    ...
    def evaluate(self, val_loader, n_val, **kwargs):
        ...
        if self.lc_weights_vis:
            plot_locally_connected_feature_maps(self.connections[('input','main1')].w,self.n_channels1,self.in_channels,0,self.crop_size,self.filter_size1,self.layers['main1'].shape[1])
            plt.show()
        ...
        if self.out_weights_vis:
            plot_locally_connected_output_weights(self.connections[('input','main1')].w,self.connections[(self.final_connection_source_name,'output')].w,\
